...
# Regression for http.response.* (HTTP/1.1): before sse.start, an application
...
subtest 'a completed decline reports the scope end to a later receive() call' => sub {
    # L<PAGI::Spec::Www> "Meaning per scope", "Receiving after the scope's
    # end": a completed refusal is a clean end, and a receive made after it
    # reports that end -- it never invents data and never invents a failure.
    # On an sse scope that is sse.disconnect with the reason key ABSENT: the
    # object was marked complete with no reason, and a reason token here
    # would look exactly like an abnormal end that never happened.
    my ($settled, $event);
    ...
    my $app = async sub {
        ...
        # Decline is now complete. Call receive() once more, WITHOUT awaiting
        # it directly, so a call that never answers leaves this subtest an
        # assertion failure rather than a hung coroutine.
        my $future = $receive->();
    ...
    ok($settled, 'the receive after the completed decline answered');
    is($event, { type => 'sse.disconnect' },
        'with the scope end, and no reason key');
