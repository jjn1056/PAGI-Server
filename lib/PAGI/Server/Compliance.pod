...
C<response_complete()> -- the spec's SHOULD-level synchronous companion to
C<on_complete>, reporting whether the response body has finished sending -- is
present but B<always returns C<undef>>: this server does not track
response-body completion. Per L<PAGI::Spec::Www/"Connection State">, C<undef>
is the documented "unsupported" signal, so applications testing C<defined>
before relying on the value behave correctly; C<response_started>
(MUST-level) and C<on_complete>/C<disconnect_future> remain fully supported.
...
or a server-initiated protocol close (C<1002>/C<1007>, reason
C<protocol_error>) -- marks the connection's disconnect-handled state so no
other path can queue a second, conflicting event. As with HTTP/2, this is a
property of the receive queue, not a hard cap on what any number of
C<receive()> calls can return: once the connection is closed, a further
C<receive()> has no queue left to read and falls back to synthesizing a
generic C<websocket.disconnect> (code C<1006>, empty C<reason>) rather than
hanging.
...
headers: C<http.response.start>, C<sse.start>, the
C<websocket.http.response> denial, the C<sse.http.response.*> decline, and
the extra headers an application supplies on C<websocket.accept>.
...
C<sse.start>, contradict the guaranteed close that follows an SSE decline
or WebSocket denial, or duplicate the server's own C<Connection: Upgrade>
...
B<completed> C<sse.http.response.*> decline (it always closes the connection
-- no event stream was ever started -- and tells a pooling client with an
explicit C<Connection: close> header so it does not reuse the socket), and
any B<abnormal> end (client disconnect, idle timeout, write error). An
abnormal end is also the only thing that delivers C<sse.disconnect> to the
application; a clean end never does. This includes a completed decline: it
is not an abnormal end, so it delivers no C<sse.disconnect> either -- an
application that calls C<receive()> again after its decline response has
already finished (before returning) gets no event at all; the call parks
rather than resolving with a synthesized disconnect.
