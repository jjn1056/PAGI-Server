...
C<sse.send> string fields and C<sse.comment> text, plus server-generated
...
framing. A Perl string that cannot be encoded fails that send's Future
instead of writing corrupted bytes to the socket. Chunk lengths, per-stream
send-queue byte counts, and transport watermarks are all measured on the
...
Keep-alive yields to the usual overrides, all of which close the connection
as before: a client C<Connection: close>, HTTP/1.0 semantics, server
shutdown, an application exception, and any B<abnormal> end (client
disconnect, idle timeout, write error). An abnormal end is also the only
