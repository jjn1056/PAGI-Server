...
C<sse.http.response.*> decline, and C<websocket.accept>.

C<TE> is the one carve-out: a C<TE> header whose value -- after trimming
leading/trailing optional whitespace (OWS) -- is exactly the token
C<trailers> is kept; any other C<TE> value, including a compound value like
C<trailers, gzip>, is stripped. Each stripped occurrence, not deduplicated
by name, logs C<PAGI: connection-specific header '$name' stripped from
HTTP/2 response (RFC 9113)>.
