...
Extended CONNECT (C<:protocol => 'websocket'>). Each accepted stream carries
...
are unaffected. An omitted C<timeout> means the server never checks for a
missing pong.

=head3 Disconnect

Each HTTP/2 WebSocket stream's scope receives exactly one
C<websocket.disconnect> event, whichever of the conditions below closes it
first. C<code> and C<reason> pair up the same way as
L<PAGI::Spec::Www/"Disconnect - receive event"> describes for the
reference server:
