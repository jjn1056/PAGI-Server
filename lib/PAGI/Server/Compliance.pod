...
leniency): nghttp2's own HTTP-messaging validation drops header B<values>
carrying leading or trailing whitespace (RFC 9113 section 8.2.1 forbids
sending them) before the request or response ever reaches PAGI application
code. An application inspecting C<scope-E<gt>{headers}> never observes a
header value with leading/trailing whitespace on HTTP/2, even one a client
actually sent -- nghttp2 has already stripped it at the C library layer.
HTTP/1.1 performs no such stripping.
...
that HTTP/2 streaming and SSE already use, rather than writing each frame
straight to nghttp2's push-style C<submit_data> entry point (which kept no
per-stream backlog for the handle to measure). From the application's
perspective the transport is invisible: a WebSocket app sees the same
C<pagi.transport> surface and semantics -- C<buffered_amount>, the high/low
watermarks, C<on_high_water>/C<on_drain> -- whether the connection is
HTTP/1.1 or HTTP/2.
...
This strip does not apply to C<http.response.trailers>: that event maps a
different, non-C<http.response.start> header list, and HTTP/1.1 trailers
already ride chunked framing the server itself controls.
