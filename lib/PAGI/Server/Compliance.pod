...
B<not implemented>. Applications must treat it as an optional capability (per
L<PAGI::Spec::Www/"Connection State">, C<undef> means unsupported) and cannot
rely on it against PAGI::Server; C<response_started> (MUST-level) and
C<on_complete>/C<disconnect_future> remain fully supported.
