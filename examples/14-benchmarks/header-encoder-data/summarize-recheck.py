...
rows=[json.loads(x) for x in (p/'post-recheck/results.jsonl').read_text().splitlines()]
resources=[json.loads(x) for x in (p/'post-recheck/resources.jsonl').read_text().splitlines()]
assert len(rows)==len(resources)==6
out={}
...
for f in sorted((p/'post-recheck').glob('*-load.txt')):
 s=f.read_text()
...
(p/'recheck-summary.json').write_text(json.dumps(summary,indent=2)+'\n')
