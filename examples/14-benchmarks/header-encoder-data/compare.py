...
assert sorted(f for f in normalized[0] if normalized[0][f]!=normalized[1][f])==['lib/PAGI/Server/Connection.pm','lib/PAGI/Server/Protocol/HTTP1.pm']
(output/'identities.json').write_text(json.dumps(identities,indent=2)+'\n')
app_hashes={str(f):hashlib.sha256(f.read_bytes()).hexdigest() for f in [b.HERE/'get.pl',b.HERE/'headers.pl',b.HERE/'post.pl',b.HERE/'stream.pl',ROOT/'examples/14-benchmarks/quiet-rerun-data/harness.py']}
(output/'app-hashes.json').write_text(json.dumps(app_hashes,indent=2)+'\n')
args=SimpleNamespace(repo=ROOT,baseline_repo=None,output=output,run_id='header-encoder',workers=1,concurrency=25,seconds=15,ws_connections=20)
original_case_config=b.case_config
b.case_config=lambda case: ('get','headers','/') if case=='headers' else original_case_config(case)
original_preflight=b.preflight
def header_preflight(port,base,path):
 original_preflight(port,base,path)
 import http.client
 conn=http.client.HTTPConnection('127.0.0.1',port,timeout=10)
 try:
  conn.request('GET','/')
  response=conn.getresponse()
  expected={'content-type':'text/plain','cache-control':'private, no-cache','vary':'accept-encoding','etag':'"benchmark-v1"','content-language':'en','x-content-type-options':'nosniff','referrer-policy':'same-origin','permissions-policy':'camera=(), microphone=()','link':'</assets/app.css>; rel=preload; as=style','x-benchmark-case':'ten-headers'}
  actual=dict((k.lower(),v) for k,v in response.getheaders())
  assert all(actual.get(k)==v for k,v in expected.items()),actual
  b.check_response(base,response)
 finally:conn.close()
for case in ['get','headers','post-observe','stream-observe']:
 b.preflight=header_preflight if case=='headers' else original_preflight
 for n,variant in enumerate(('release','baseline','candidate','candidate','baseline','release')):
  ...
  total=row['responses']+b.parse_hey(warmfile.read_text())['responses']+(4 if case in ('post-observe','headers') else 3)
  ...
  for f,h in app_hashes.items():assert hashlib.sha256(Path(f).read_bytes()).hexdigest()==h,f
  for ident in identities.values():
   for f,h in ident['sha256'].items():assert hashlib.sha256(Path(f).read_bytes()).hexdigest()==h,f
print('All 24 runs passed with unchanged source hashes.',flush=True)
