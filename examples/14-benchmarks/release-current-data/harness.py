...
def run_one(args, variant, case, env, perl, installed, server_env=None):
    ...
    command = ['/usr/bin/time', '-l', '-o', str(args.output / (stamp + '.process-time.txt')), *command]
    with (args.output / (stamp + '.server.log')).open('w') as log:
        proc = subprocess.Popen(command, cwd=args.repo, env=server_env if server_env is not None else env, stdout=log, stderr=log, start_new_session=True)
        try:
            for _ in range(100):
