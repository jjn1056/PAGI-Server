...
rows=[json.loads(x) for x in (p/'stream-recheck/results.jsonl').read_text().splitlines()]
resources=[json.loads(x) for x in (p/'stream-recheck/resources.jsonl').read_text().splitlines()]
...
for f in sorted((p/'stream-recheck').glob('*-load.txt')):
 s=f.read_text()
