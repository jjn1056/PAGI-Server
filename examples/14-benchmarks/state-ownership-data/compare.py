...
sources={'release':None,'before':exp/'before','candidate':exp/'candidate'}
...
expected=json.loads((exp/'candidate-hashes.json').read_text());assert hashes['candidate']==expected
...
meta={'base':'6dbcf47','runtime_base':'874b120','sources':hashes,'variants':{},'env':{k:env.get(k) for k in ['LIBEV_FLAGS','PERL_FUTURE_NO_XS','GOMAXPROCS']},'server_cpu':0,'client_cpus':[2,3],'driver_cpus':sorted(os.sched_getaffinity(0))}
...
orders=[['release','before','candidate'],['candidate','before','release'],['before','release','candidate']]
