...
out=exp/'batch-sweep';out.mkdir(exist_ok=False)
...
for size in [1,16,256]:
 path=exp/('batch'+str(size));shutil.copytree(exp/'listener',path)
 module=path/'lib/PAGI/Server/Listener.pm'
 source=module.read_text();assert source.count('use constant ACCEPT_BATCH => 64;')==1
 module.write_text(source.replace('use constant ACCEPT_BATCH => 64;',f'use constant ACCEPT_BATCH => {size};'))
sources={'release':None,'before':exp/'before','batch1':exp/'batch1','batch16':exp/'batch16','listener':exp/'listener','batch256':exp/'batch256'}
...
expected=json.loads((exp/'identities.json').read_text())
assert all(hashes[k]==v for k,v in expected.items())
for label in ['batch1','batch16','batch256']:
 assert [k for k in hashes[label] if hashes[label][k]!=hashes['listener'][k]]==['lib/PAGI/Server/Listener.pm']
...
cases=['storm']
orders=[['release','before','batch1','batch16','listener','batch256'],
        ['batch256','listener','batch16','batch1','before','release'],
        ['batch16','release','batch256','before','batch1','listener']]
meta.update(cases=cases,orders=orders,smoke=False,
 settings={'background_seconds':12,'concurrency':500,'background_concurrency':100,'batch_sizes':[1,16,64,256]})
