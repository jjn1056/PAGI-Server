...
def case_config(case):
    if case in ('get-500', 'churn', 'storm'):
        return 'get', 'get', '/'
...
def parse_hey(output):
    ...
    patterns = {'rps': r'Requests/sec:\s+([\d.]+)', 'average_s': r'Average:\s+([\d.]+)',
                'p50_s': r'50% in ([\d.]+)', 'p95_s': r'95% in ([\d.]+)',
...
def run_one(args, variant, case, env, perl, installed):
    ...
    command = ['taskset', '-c', '0', '/usr/bin/time', '-f', 'user_s=%U system_s=%S maxrss_kib=%M major_faults=%F minor_faults=%R', '-o', str(args.output / (stamp + '.resources.txt')), *command]
    with (args.output / (stamp + '.server.log')).open('w') as log:
        ...
        try:
            ...
            if base == 'websocket':
                client = ['taskset', '-c', '2,3', perl, str(HERE / 'ws-client.pl'), f'ws://127.0.0.1:{port}/', str(args.seconds), str(args.ws_connections)]
